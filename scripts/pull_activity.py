...
def authored_prs(user: str, repo: str, cutoff: datetime) -> list[dict]:
...
def authored_issues(user: str, cutoff: datetime) -> list[dict]:
    cutoff_date = cutoff.date().isoformat()
    search_query = f"author:{user} is:issue created:>={cutoff_date}"
    query = """
    {
      search(query: "%s", type: ISSUE, first: 50) {
        nodes {
          ... on Issue {
            number
            title
...
def reviews_given(user: str, cutoff: datetime) -> list[dict]:
    """Each review: body, state, PR context, and every inline line comment.

    Uses GraphQL contributionsCollection + the underlying PullRequestReview
    nodes. Inline comments attached to the review are returned per-review.
    """
    query = """
    {
      user(login: "%s") {
    ...
    """ % user
    data = graphql(query)
    nodes = data["data"]["user"]["contributionsCollection"][
        "pullRequestReviewContributions"
    ]["nodes"]
    out = []
    for n in nodes:
        occurred = datetime.fromisoformat(n["occurredAt"].replace("Z", "+00:00"))
        if occurred < cutoff:
            continue
        pr = n["pullRequest"]
        review = n["pullRequestReview"] or {}
        inline_comments = (review.get("comments") or {}).get("nodes") or []
        out.append(
            {
...
def issue_comments(user: str, cutoff: datetime) -> list[dict]:
    ...
    """ % user
    data = graphql(query)
    nodes = data["data"]["user"]["issueComments"]["nodes"]
    out = []
    for n in nodes:
...
def local_commits(
    git_author: str, repo_path: str, cutoff: datetime
) -> list[dict]:
...
def main() -> int:
    ...
    authored = []
    for repo in repos:
        authored.extend(authored_prs(args.user, repo, cutoff))
    output["authored_prs"] = sorted(
        authored, key=lambda p: p["updatedAt"], reverse=True
    )
    output["authored_issues"] = authored_issues(args.user, cutoff)
    output["reviews_given"] = reviews_given(args.user, cutoff)
    output["issue_comments"] = issue_comments(args.user, cutoff)
    ...
    if local_paths:
        ...
        for path in local_paths:
            ...
            commits_by_repo[resolved] = local_commits(
                args.git_author, resolved, cutoff
            )
